...
if __name__ == "__main__":
    ...
    model = model.to(device) # device 已经在前面定义为 torch.device("cuda")
    ...
    for epoch in range(epochs):
        ...
        for data in train_loader:
            # 原本的这行 break 逻辑如果是为了跳过训练，请根据你的需求保留或检查逻辑
            # if epoch < end_epoch: break 

            # 核心修改 A：将整个 Batch 一次性推送到显卡
            # 此时 data 是一个 Batch 对象，不再是 list
            data = data.to(device) 

            # 核心修改 B：不再需要手动拼接
            # 在标准的 DataLoader 中，data.y 已经是拼接好的 Tensor 了
            y = data.y.view(-1, out_channels) 

            optimizer.zero_grad()

            # 核心修改 C：直接传入 data 对象
            # 此时 data 拥有 .time_step_len 等所有模型需要的属性
            out = model(data)
