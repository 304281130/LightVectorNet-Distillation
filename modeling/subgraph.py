...
class SubGraph(nn.Module):
    ...
    def forward(self, sub_data):
        # 🚀 增加一步：不直接修改 sub_data，而是先创建一个浅拷贝
        # 这样即便我们修改了 out_data.x，也不会影响到原来的 batch_data
        out_data = sub_data.clone() 
        
        x, edge_index = out_data.x, out_data.edge_index
        ...
        out_data.x = x
        # 聚合特征
        out_data = max_pool(out_data.cluster, out_data)
        
        # L2 归一化
        eps = 1e-8
        norm = out_data.x.norm(p=2, dim=1, keepdim=True)
        out_data.x = out_data.x / (norm + eps)
