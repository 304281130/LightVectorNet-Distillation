...
class HGNN(nn.Module):
    ...
    def forward(self, data):
        # 1. 局部特征提取
        # sub_graph_out.x 此时是 [57, 64] (假设 16 个样本总共 57 条折线)
        sub_graph_out = self.subgraph(data)
        
        # 2. 获取批次大小
        bs = data.valid_len.shape[0] 
        
        # 3. 【核心修复】使用聚合后的 batch 信息
        # 必须用 sub_graph_out.batch，它才是对应那 57 条折线的正确归属！
        from torch_geometric.utils import to_dense_batch
        
        # x 形状: [bs, Max_Polylines_in_Batch, 64]
        # mask 形状: [bs, Max_Polylines_in_Batch]
        x, mask = to_dense_batch(
        sub_graph_out.x, 
        sub_graph_out.batch, 
        batch_size=data.valid_len.size(0) # 强制要求输出 16 个样本
        )
        
        # 4. 重新计算注意力掩码需要的有效长度
        # 统计每一行中 True 的个数，即为每个样本真实的折线数量
