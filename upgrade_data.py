...
def upgrade_pyg_data(file_path):
    if not os.path.exists(file_path):
        print(f"[错误] 找不到文件: {file_path} (请确认你已经重新解压了数据)")
        return
...
if __name__ == "__main__":
    # 分别升级训练集和验证集
    upgrade_pyg_data('./interm_data/train_intermediate/processed/dataset.pt')
    upgrade_pyg_data('./interm_data/val_intermediate/processed/dataset.pt')
